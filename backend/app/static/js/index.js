import { enrichWork, getEpisodes, getStatus, getWork, getWorks } from './api.js';
...
function statusBadge(status) {
  return text('span', status === 'active' ? '上架' : '下架', `badge badge-${status === 'active' ? 'active' : 'removed'}`);
}
...
async function loadWorks(page = 1) {
  ...
  setStatus(message, '');
  try {
    const data = await getWorks({ q: currentQuery, status: currentStatus, tag: currentTag, page, pageSize: PAGE_SIZE });
...
async function queueEnrichment(work, button, statusNode) {
  button.disabled = true;
  setStatus(statusNode, '正在创建公开详情刷新任务…');
  try {
    const task = await enrichWork(work.id);
    setStatus(statusNode, `任务已创建：${task.task_id}。可在“任务与导入”页面查看进度。`, 'success');
  } catch (error) {
    setStatus(statusNode, errorMessage(error), 'error');
...
async function openDetail(seriesId) {
  ...
  try {
    ...
    const actionMessage = text('div', '', 'status-line mt-8');
    if (work.source === 'novelquick') {
      const enrichButton = text('button', '刷新公开详情与分集', 'btn btn-primary');
      enrichButton.type = 'button';
      enrichButton.addEventListener('click', () => queueEnrichment(work, enrichButton, actionMessage));
      actions.append(enrichButton);
    }
    if (actions.childNodes.length) body.append(actions, actionMessage);

    layout.append(body);
    detailContent.append(layout);
    detailContent.append(text('h3', '分集', 'mt-16'));
    if (!episodes.length) {
      detailContent.append(text('p', '尚未导入分集明细。可使用“刷新公开详情与分集”创建后台任务。', 'text-muted mt-8'));
    } else {

...
- 一个 `android/app` module，同时支持普通 Launcher 和 Leanback Launcher；
...
生产环境目前没有注册 playback provider。Android 详情/分集导航、Media3 播放、NAS 代理 handoff、设备端下载和真机测试尚未完成。
...
构建容器固定 JDK 17、Android SDK Platform 35、Build Tools 35.0.0 和 Gradle 8.7。首次构建会下载较大的 SDK、Gradle 和 Maven 依赖，但都保存在 Docker/Gradle 缓存中，不污染宿主机。
...
CI 每次运行 JVM 单元测试和 `assembleDebug`，并把 APK 上传为 14 天保留的 workflow artifact；APK 不提交 Git。
...
地址保存在应用私有 SharedPreferences。当前 Manifest 允许 cleartext 以支持局域网 Debug；URL 校验阻止公网明文 HTTP，Release 网络安全策略后续收紧。
